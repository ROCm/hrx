...
struct BenchmarkContext {
  // Observations required by the fixed native test module.
...
void DeinitializeBenchmarkContext(BenchmarkContext* context) {
  ...
  iree_vm_module_release(&context->module.base);
}

iree_status_t InitializeBenchmarkContext(BenchmarkContext* context) {
  IREE_RETURN_IF_ERROR(iree_vm_invocation_test_module_initialize(
      &context->counters, &context->module));
  iree_status_t status = iree_vm_program_create(
      {&context->module.base, iree_vm_module_span_empty()},
