...
#include "iree/vm/bytecode/interpreter_tables.inl"
...
// and Wasm use one shared portable switch over the same handler labels.
...
#define IREE_VM_BYTECODE_DISPATCH() \
  goto*((const uint8_t*) &&         \
        iree_vm_bytecode_dispatch_invalid + dispatch_table[record_data[0]])
#else
// The portable switch uses goto only to reach direct-threaded handlers. Every
// instruction tail returns to this one switch so the compiler sees O(opcodes)
// dispatch edges instead of expanding an O(opcodes) switch at every handler.
...
#define IREE_VM_BYTECODE_DISPATCH_BEGIN()             \
  iree_vm_bytecode_dispatch_switch:                   \
...
#define IREE_VM_BYTECODE_DISPATCH() goto iree_vm_bytecode_dispatch_switch
