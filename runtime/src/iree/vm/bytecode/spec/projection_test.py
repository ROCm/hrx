...
class ProjectionTest(unittest.TestCase):
    def setUp(self) -> None:
        self.outputs = {
            ...
            "instruction_verifier_cases": render_instruction_verifier_cases(
                SPECIFICATION
            ),
    ...
    def test_assembler_projects_terminal_blob_boundaries(self) -> None:
        assembler = self.outputs["assembler_data"]
        self.assertIn("iree_vm_bytecode_assembler_terminal_offsets[]", assembler)
        self.assertEqual(
            assembler.count(
                "static const iree_vm_bytecode_assembler_terminal_offset_t"
            ),
            1,
        )
