...
_LIFECYCLE_CONTRACT = (
    "Creation from externally supplied bytecode is the single verification boundary "
    "for every fact knowable from one image and its ref-type environment. It validates "
    "bounded planning fields before allocation, derives exact resolved-ref and "
    "rodata-root multiplicities, validates every section and cross-section relation, "
    "allocates one module slab, resolves ref types into that slab, and publishes "
    "nothing until every check succeeds. Trusted in-process creation follows the same "
    "bounds-safe mapping, runtime compatibility, allocation, type-resolution, and "
    "ownership path but relies on its compiler producer for canonical section and "
    "instruction semantics. It is valid only when that producer is part of the trusted "
    "computing base; files, caches, network data, and other externally supplied images "
    "require verified creation. Only the module name, flat resolved-type handles, and "
    "rodata root objects need variable allocation tails; declarations and instructions "
    "remain mapped and require no decoded-table copies. Structural verification proves "
    "bytes, versions, ordinals, extents, declarations, instruction records, and exact "
    "control targets. It deliberately does not build a call graph, propagate may-yield, "
    "prove reachability or POD definite assignment, or compute ref-ownership dataflow. "
    ...
    "Mutation after verification or trusted production is outside the VM contract and "
    "execution performs no per-instruction integrity check. This is not a sandbox "
    "boundary; isolation between mutually untrusted programs belongs at the process or "
    "machine boundary."
)
