...
class ModuleVerificationTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    contents_ = iree_const_cast_byte_span(fixture_.contents);
...
class CoreExecutionModuleVerificationTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(CoreExecutionModuleVerificationTest, MapsAndVerifies) {
  const iree_const_byte_span_t contents = this->contents();
...
TEST_F(CoreExecutionModuleVerificationTest,
       RejectsDistinctRepresentativesForEqualCallableSignatures) {
  const iree_const_byte_span_t contents = this->contents();
  iree_vm_bytecode_module_plan_t plan = {};
  IREE_ASSERT_OK(iree_vm_bytecode_verify_module_structure(contents, &plan));
  ASSERT_GT(plan.layout.signatures.count, 9u);
  ASSERT_GT(plan.layout.callable_types.count, 2u);

  std::vector<uint8_t> bytes(contents.data,
                             contents.data + contents.data_length);
  MutableRecord(&bytes, contents, &plan.layout.callable_types.rows[2])
      ->signature_ordinal_u16 = 9;
  IREE_EXPECT_STATUS_IS(
...
TEST_F(ModuleVerificationTest, MapsCompleteCanonicalImage) {
  EXPECT_EQ(plan_.layout.image.section_count, 13u);
...
TEST_F(ModuleVerificationTest, FullVerificationPublishesOnlyOnSuccess) {
  iree_vm_bytecode_module_plan_t output = {};
  IREE_ASSERT_OK(iree_vm_bytecode_verify_module(
      contents_, iree_allocator_system(), &output));
  EXPECT_EQ(output.layout.image.header, plan_.layout.image.header);

  std::vector<uint8_t> bytes = CopyImage();
  const uint8_t* original = FindInstruction(IREE_VM_BYTECODE_OPCODE_VALUE_COPY);
  ASSERT_NE(original, nullptr);
  MutableByte(&bytes, contents_, original)[0] = 0;
  output.layout.image.header =
      reinterpret_cast<const iree_vm_bytecode_v0_image_header_t*>(uintptr_t{1});
  output.process_layout.total_size = 123;
  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_INVALID_ARGUMENT,
      iree_vm_bytecode_verify_module(
          iree_make_const_byte_span(bytes.data(), bytes.size()),
          iree_allocator_system(), &output));
  EXPECT_EQ(output.layout.image.header,
            reinterpret_cast<const iree_vm_bytecode_v0_image_header_t*>(
                uintptr_t{1}));
  EXPECT_EQ(output.process_layout.total_size, 123u);
...
TEST_F(ModuleVerificationTest, RejectsInvalidStatusSelector) {
  std::vector<uint8_t> bytes = CopyImage();
  const uint8_t* original =
...
TEST_F(ModuleVerificationTest, RejectsF32MathSelectorInF64Instruction) {
  std::vector<uint8_t> bytes = CopyImage();
  const uint8_t* original =
      FindInstruction(IREE_VM_BYTECODE_OPCODE_FLOAT_MATH_UNARY_F64);
  ASSERT_NE(original, nullptr);
  auto* math = MutableRecord(
      &bytes, contents_,
      reinterpret_cast<const iree_vm_bytecode_float_math_unary_f64_t*>(
          original));
  math->selector_u8 = IREE_VM_BYTECODE_FLOAT_MATH_F32_EXP2_APPROX;
  ExpectInstructionsRejected(std::move(bytes));
}
...
TEST_F(ModuleVerificationTest, PlansDirectAccessForAlignedRodata) {
  ...
  const iree_host_size_t offset = (64u - (storage_address & 63u)) & 63u;
  uint8_t* image = storage.data() + offset;
  ASSERT_EQ(reinterpret_cast<uintptr_t>(image) & 63u, 0u);
...
TEST_F(ModuleVerificationTest, AcceptsUnknownSkippableSection) {
  std::vector<uint8_t> bytes = CopyImage();
  auto* row = MutableRecord(&bytes, contents_,
                            FindSection(IREE_VM_BYTECODE_SECTION_METADATA));
  row->section_type_u16 = UINT16_C(0xF001);

  iree_vm_bytecode_module_plan_t plan = {};
  IREE_ASSERT_OK(iree_vm_bytecode_verify_module_structure(
      iree_make_const_byte_span(bytes.data(), bytes.size()), &plan));
  EXPECT_EQ(plan.layout.metadata.header, nullptr);
}

TEST_F(ModuleVerificationTest, RejectsEnvelopeAndDirectoryCorruption) {
  {
    SCOPED_TRACE("image magic");
    std::vector<uint8_t> bytes = CopyImage();
  ...
  {
    SCOPED_TRACE("core major version");
    std::vector<uint8_t> bytes = CopyImage();
  ...
  {
    SCOPED_TRACE("known section flags");
    std::vector<uint8_t> bytes = CopyImage();
  ...
  {
    SCOPED_TRACE("required unknown section");
    std::vector<uint8_t> bytes = CopyImage();
    auto* extension_row = MutableRecord(
        &bytes, contents_, FindSection(IREE_VM_BYTECODE_SECTION_METADATA));
    extension_row->section_type_u16 = UINT16_C(0xF001);
    extension_row->section_flags_u16 = 0;
    ExpectRejected(bytes, IREE_STATUS_INCOMPATIBLE);
  }
  {
    SCOPED_TRACE("invalid requirement page");
    std::vector<uint8_t> bytes = CopyImage();
    auto* requirement =
        MutableRecord(&bytes, contents_, plan_.layout.requirements.rows);
    requirement->page_id_u16 = UINT16_C(0xEF);
    ExpectRejected(bytes);
  }
  {
    SCOPED_TRACE("trailing image data");
    std::vector<uint8_t> bytes = CopyImage();
    bytes.push_back(0);
    ExpectRejected(bytes);
  }
  {
    SCOPED_TRACE("truncated image data");
    std::vector<uint8_t> bytes = CopyImage();
    bytes.pop_back();
    ExpectRejected(bytes);
  }
