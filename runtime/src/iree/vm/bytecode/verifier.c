...
#include "iree/vm/bytecode/verifier_module_cases.inl"
    default:
...
static iree_status_t iree_vm_bytecode_verify_callable_types(
  ...
  for (uint32_t i = 0; i < layout->callable_types.count; ++i) {
    ...
    if (i != 0) {
      ...
      if (iree_vm_bytecode_compare_callable_types(layout, &rows[i - 1], row,
                                                  &has_same_signature) >= 0) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT,
            "callable types must be unique and strictly ordered");
      }
      if (has_same_signature &&
          rows[i - 1].signature_ordinal_u16 != row->signature_ordinal_u16) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT,
            "equal callable signatures must share one signature ordinal");
      }
    }
  }
  for (uint32_t i = 0; i < layout->signatures.descriptor_count; ++i) {
...
static bool iree_vm_bytecode_verify_instruction(
    ...
    const iree_vm_bytecode_instruction_context_t* context) {
  switch (record[0]) {
#include "iree/vm/bytecode/verifier_instruction_cases.inl"
    default:
...
iree_status_t iree_vm_bytecode_verify_module(
  ...
  if (maximum_block_count > IREE_ARRAYSIZE(inline_block_offsets)) {
    status = iree_allocator_malloc_array_uninitialized(
        scratch_allocator, maximum_block_count, sizeof(*block_offsets),
        (void**)&block_offsets);
  }
