...
#include "iree/vm/bytecode/interpreter_conversion_test_vectors.inl"
...
TEST(VMBytecodeInterpreterConversionTest, ExecutesEveryIntegerSelector) {
  bool selector_seen[IREE_VM_BYTECODE_INTEGER_CONVERT_I64_TO_I32 + 1] = {};
  for (const ConversionTestRow& row : kIntegerTestRows) {
    ...
    EXPECT_EQ(ExecuteConversion<iree_vm_bytecode_conversion_integer_t>(
...
TEST(VMBytecodeInterpreterConversionTest,
  ...
  for (uint32_t bits = 0; bits < 256; ++bits) {
    const uint32_t actual_e4 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_extend_t>(
            iree_vm_bytecode_execute_conversion_float_extend,
            IREE_VM_BYTECODE_FLOAT_EXTEND_F8E4M3_TO_F32, bits);
    ...
    const uint32_t actual_e5 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_extend_t>(
            iree_vm_bytecode_execute_conversion_float_extend,
            IREE_VM_BYTECODE_FLOAT_EXTEND_F8E5M2_TO_F32, bits);
  ...
  for (uint32_t bits = 0; bits < 65536; ++bits) {
    const uint32_t actual_f16 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_extend_t>(
            iree_vm_bytecode_execute_conversion_float_extend,
            IREE_VM_BYTECODE_FLOAT_EXTEND_F16_TO_F32, bits);
    ...
    const uint32_t actual_bf16 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_extend_t>(
            iree_vm_bytecode_execute_conversion_float_extend,
            IREE_VM_BYTECODE_FLOAT_EXTEND_BF16_TO_F32, bits);
...
TEST(VMBytecodeInterpreterConversionTest, RoundsEveryFloatBoundaryWitness) {
  bool truncate_selector_seen[IREE_VM_BYTECODE_FLOAT_TRUNCATE_F64_TO_BF16 + 1] =
      {};
  for (const ConversionTestRow& row : kFloatTruncateTestRows) {
    ...
    EXPECT_EQ(ExecuteConversion<iree_vm_bytecode_conversion_float_truncate_t>(
  ...
  bool width_selector_seen[IREE_VM_BYTECODE_FLOAT_WIDTH_F64_TO_F32 + 1] = {};
  for (const ConversionTestRow& row : kFloatWidthTestRows) {
    ...
    EXPECT_EQ(ExecuteConversion<iree_vm_bytecode_conversion_float_width_t>(
...
TEST(VMBytecodeInterpreterConversionTest,
  ...
  for (uint32_t i = 0; i < kSampleCount; ++i) {
    ...
    const uint8_t actual_e4 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_truncate_t>(
            iree_vm_bytecode_execute_conversion_float_truncate,
            IREE_VM_BYTECODE_FLOAT_TRUNCATE_F32_TO_F8E4M3, source_bits);
    ...
    const uint8_t actual_e5 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_truncate_t>(
            iree_vm_bytecode_execute_conversion_float_truncate,
            IREE_VM_BYTECODE_FLOAT_TRUNCATE_F32_TO_F8E5M2, source_bits);
    ...
    const uint16_t actual_f16 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_truncate_t>(
            iree_vm_bytecode_execute_conversion_float_truncate,
            IREE_VM_BYTECODE_FLOAT_TRUNCATE_F32_TO_F16, source_bits);
    ...
    const uint16_t actual_bf16 =
        ExecuteConversion<iree_vm_bytecode_conversion_float_truncate_t>(
            iree_vm_bytecode_execute_conversion_float_truncate,
            IREE_VM_BYTECODE_FLOAT_TRUNCATE_F32_TO_BF16, source_bits);
...
TEST(VMBytecodeInterpreterConversionTest, RoundsIntegersDirectly) {
  bool selector_seen[IREE_VM_BYTECODE_INTEGER_TO_FLOAT_U64_TO_BF16 + 1] = {};
  for (const ConversionTestRow& row : kIntegerToFloatTestRows) {
    SCOPED_TRACE(static_cast<int>(row.selector));
    ASSERT_LT(row.selector, IREE_ARRAYSIZE(selector_seen));
    selector_seen[row.selector] = true;
    EXPECT_EQ(ExecuteConversion<iree_vm_bytecode_conversion_integer_to_float_t>(
  ...
  const uint64_t direct =
      ExecuteConversion<iree_vm_bytecode_conversion_integer_to_float_t>(
          iree_vm_bytecode_execute_conversion_integer_to_float,
          IREE_VM_BYTECODE_INTEGER_TO_FLOAT_U64_TO_BF16,
          kDoubleRoundingWitness);
...
TEST(VMBytecodeInterpreterConversionTest,
  ...
  bool selector_seen[IREE_VM_BYTECODE_FLOAT_TO_INTEGER_F64_TO_U64 + 1] = {};
