...
struct BenchmarkContext {
  // Owned assembled launch-config module storage.
  iree_vm_bytecode_module_fixture_t module_fixture = {};
...
iree_vm_bytecode_module_storage_t MakeModuleStorage(
    const iree_vm_bytecode_module_fixture_t* fixture) {
  return {
...
void DeinitializeBenchmarkContext(BenchmarkContext* context) {
  ...
  iree_vm_bytecode_module_fixture_deinitialize(&context->module_fixture);
  *context = {};
}
...
iree_status_t InitializeBenchmarkContext(BenchmarkContext* context) {
  ...
  iree_status_t status = iree_vm_bytecode_module_fixture_initialize(
      iree_make_string_view(reinterpret_cast<const char*>(files[0].data),
                            files[0].size),
      iree_allocator_system(), &context->module_fixture);
  if (iree_status_is_ok(status)) {
    status = iree_vm_environment_allocate(iree_allocator_system(),
                                          &context->environment);
  }
  if (iree_status_is_ok(status)) {
    status = iree_vm_bytecode_module_create(
        context->environment, IREE_SV("launch"),
        MakeModuleStorage(&context->module_fixture), iree_allocator_system(),
        &context->module);
  }
  if (iree_status_is_ok(status)) {
    status =
...
iree_status_t RunModuleCreateBenchmark(
  ...
  iree_status_t status = InitializeBenchmarkContext(&context);
