...
#include "iree/hal/drivers/task/command/block_builder.h"
#include "iree/hal/drivers/task/command/block_command_buffer.h"
#include "iree/hal/drivers/task/command/block_command_ops.h"
...
static iree_status_t iree_hal_task_queue_profile_set_dispatch(
  ...
  profile_operation->executable_id =
      iree_hal_task_executable_profile_id(task_executable);
...
static void iree_hal_task_queue_trace_compute_drain_event(void) {
  IREE_TRACE_ZONE_BEGIN_NAMED(z_drain, "iree_hal_task_compute_drain");
...
static iree_status_t iree_hal_task_queue_barrier(
    iree_hal_queue_t* base_queue,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
...
static iree_status_t iree_hal_task_queue_execute(
  ...
  return iree_hal_task_queue_submit_commands((iree_hal_task_queue_t*)base_queue,
                                             /*batch_count=*/1, &batch);
}
