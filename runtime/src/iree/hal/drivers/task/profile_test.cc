...
typedef struct FakeTaskExecutable {
  iree_hal_task_executable_t base;
} FakeTaskExecutable;

static void FakeTaskExecutableDestroy(iree_hal_executable_t* executable) {
  (void)executable;
}

static iree_host_size_t FakeTaskExecutableFunctionCount(
    iree_hal_executable_t* executable) {
  (void)executable;
  return 2;
}

static iree_status_t FakeTaskExecutableFunctionInfo(
...
static iree_status_t FakeTaskExecutableFunctionParameters(
    iree_hal_executable_t* executable,
...
static iree_status_t FakeTaskExecutableLookupFunctionByName(
...
static iree_status_t FakeTaskExecutableIssueCall(
    iree_hal_task_executable_t* executable, iree_host_size_t ordinal,
...
static const iree_hal_task_executable_vtable_t kFakeTaskExecutableVTable = {
    {
        FakeTaskExecutableDestroy,
        FakeTaskExecutableFunctionCount,
        FakeTaskExecutableFunctionInfo,
        FakeTaskExecutableFunctionParameters,
        FakeTaskExecutableLookupFunctionByName,
    },
    FakeTaskExecutableIssueCall,
};
...
TEST_F(TaskProfileRecorderTest, RecordsExecutableMetadataOnce) {
  ...
  iree_hal_task_executable_deinitialize(&executable.base);
}
