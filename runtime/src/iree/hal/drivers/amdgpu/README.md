...
iree-run-loom --device=amdgpu://GPU-0e12865a3bf5b7ab --probe-hal
# Single logical device with the two devices given by their UUIDs:
iree-run-loom \
  --device=amdgpu://GPU-0e12865a3bf5b7ab,GPU-89e8bdf59a10cf6d \
  --probe-hal
...
The device normally selects the most specific compatible compiler target. Use
an explicit target such as `--target=amdgpu:gfx11-generic` on compatible gfx11
hardware when the compiled artifact must remain generic.
