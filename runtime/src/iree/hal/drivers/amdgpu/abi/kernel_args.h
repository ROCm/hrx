...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_amdgpu_kernel_implicit_args_t {
  ...
  // Grid synchronization state for cooperative dispatches. Single-grid
  // synchronization uses this field despite its historical metadata name.
  //
  // Represented in metadata as:
...
// Synchronization words shared by all workgroups in one cooperative grid.
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_amdgpu_grid_sync_data_t {
  // First device-library-owned synchronization word.
  uint32_t word0;
  // Second device-library-owned synchronization word.
  uint32_t word1;
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_amdgpu_grid_sync_info_t {
  // Device address of synchronization words shared across grids, or zero for
  // a single-grid dispatch.
  uint64_t multi_grid_sync;
  // Zero-based grid ordinal within a multi-grid launch.
