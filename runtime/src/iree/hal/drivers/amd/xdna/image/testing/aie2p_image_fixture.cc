...
static std::vector<uint8_t> EncodeControl(bool use_relocatable_block_write) {
  std::vector<uint8_t> record;
  if (use_relocatable_block_write) {
    constexpr uint32_t kWords[] = {0, 0};
    record.resize(24);
    IREE_CHECK_OK(iree_hal_amd_xdna_aie2p_encode_register_block_write32(
        UINT32_C(0x0001D000), std::size(kWords), kWords,
...
std::vector<uint8_t> BuildAie2pImage(const Aie2pImageFixtureOptions& options) {
  ...
  const std::vector<uint8_t> control =
      EncodeControl(options.use_relocatable_block_write);
