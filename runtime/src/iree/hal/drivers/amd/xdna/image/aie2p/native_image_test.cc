  ...
  while (byte_offset < transaction.data_length) {
    ...
    switch (opcode) {
      ...
      case 128:
        ...
        break;
      case 129:
        byte_length = IREE_HAL_AMD_XDNA_AIE2P_TRANSACTION_ADDRESS_PATCH_SIZE;
        break;
      default:
...
TEST(Aie2pNativeImageTest, LowersAndOwnsSyntheticImage) {
  ...
  ASSERT_EQ(entry.control.data_length, 88u);
  ExpectTransactionHeader(entry.control, 2, 3);
  ...
  EXPECT_EQ(entry.control.data[40], 129u);
  EXPECT_EQ(iree_unaligned_load_le_u64(entry.control.data + 64),
            UINT64_C(0x0001D000));
  EXPECT_EQ(iree_unaligned_load_le_u64(entry.control.data + 72), 0u);
  EXPECT_EQ(iree_unaligned_load_le_u64(entry.control.data + 80), 0u);
...
TEST(Aie2pNativeImageTest, LowersCanonicalMulI32Image) {
  ...
  ExpectTransactionHeader(entry.control, 11, 1);
  ...
  constexpr uint64_t kPatchArguments[] = {0, 1, 2};
  constexpr uint64_t kPatchAddends[] = {0, 0, 0};
  iree_host_size_t patch_ordinal = 0;
  for (const NativeOperation& operation : control_operations) {
    if (operation.opcode != 129) continue;
    ASSERT_LT(patch_ordinal, std::size(kPatchAddresses));
    EXPECT_EQ(iree_unaligned_load_le_u64(entry.control.data +
                                         operation.byte_offset + 24),
              kPatchAddresses[patch_ordinal]);
    EXPECT_EQ(iree_unaligned_load_le_u64(entry.control.data +
                                         operation.byte_offset + 32),
              kPatchArguments[patch_ordinal]);
    EXPECT_EQ(iree_unaligned_load_le_u64(entry.control.data +
                                         operation.byte_offset + 40),
              kPatchAddends[patch_ordinal]);
    ++patch_ordinal;
  }
  EXPECT_EQ(patch_ordinal, std::size(kPatchAddresses));
}
