...
static iree_status_t iree_hal_amd_xdna_aie2p_native_query_argument_ordinal(
    const iree_hal_amd_xdna_image_t* image,
    const iree_hal_amd_xdna_elf_entry_record_t* entry, uint32_t binding_ordinal,
    uint64_t* out_argument_ordinal) {
  *out_argument_ordinal = 0;
  const uint64_t binding_end =
      (uint64_t)entry->first_binding_ordinal + entry->binding_count;
  if (binding_ordinal < entry->first_binding_ordinal ||
      binding_ordinal >= binding_end) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "AIE2P CONTROL relocation references another entry's binding");
  }
  uint64_t argument_ordinal = 0;
  for (uint64_t i = entry->first_binding_ordinal; i <= binding_ordinal; ++i) {
    const iree_hal_amd_xdna_elf_binding_record_t* binding =
        iree_hal_amd_xdna_image_binding(image, (iree_host_size_t)i);
    if (binding == NULL || binding->entry_ordinal != entry->export_ordinal) {
      return iree_make_status(
          IREE_STATUS_FAILED_PRECONDITION,
          "qualified AIE2P entry has an inconsistent binding range");
    }
    if (i == binding_ordinal) {
      if (binding->kind != IREE_HAL_AMD_XDNA_ELF_BINDING_KIND_BUFFER) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT,
            "AIE2P native address patch requires a buffer binding");
      }
      *out_argument_ordinal = argument_ordinal;
      return iree_ok_status();
    }
    if (binding->kind == IREE_HAL_AMD_XDNA_ELF_BINDING_KIND_BUFFER) {
      ++argument_ordinal;
  ...
  IREE_CHECK_UNREACHABLE("validated AIE2P binding ordinal");
}
...
static iree_status_t iree_hal_amd_xdna_aie2p_native_measure_control(
  ...
  *out_measurement = (iree_hal_amd_xdna_aie2p_native_measurement_t){
  ...
  for (iree_host_size_t i = first_relocation; i < relocation_end; ++i) {
    const iree_hal_amd_xdna_elf_relocation_record_t* relocation =
        iree_hal_amd_xdna_image_relocation(image, i);
    uint64_t argument_ordinal = 0;
    IREE_RETURN_IF_ERROR(iree_hal_amd_xdna_aie2p_native_query_argument_ordinal(
        image, entry, relocation->binding_ordinal, &argument_ordinal));
    IREE_RETURN_IF_ERROR(iree_hal_amd_xdna_aie2p_native_measurement_add(
        IREE_HAL_AMD_XDNA_AIE2P_TRANSACTION_ADDRESS_PATCH_SIZE, 0,
        out_measurement));
  }
  return iree_ok_status();
}
...
static iree_status_t iree_hal_amd_xdna_aie2p_native_append_record_relocations(
    ...
    iree_hal_amd_xdna_aie2p_transaction_writer_t* writer) {
  if (record->source_range.offset < control_header->file_range.offset) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "qualified AIE2P CONTROL record has an invalid source range");
  }
  ...
  while (*relocation_ordinal < relocation_end) {
    ...
    if (decoded_record->type !=
      ...
      return iree_make_status(
          IREE_STATUS_FAILED_PRECONDITION,
          "qualified AIE2P relocation changed during native lowering");
    }
    ...
    if (!iree_checked_add_u64(
            decoded_record->value.register_block_write32.address,
            word_byte_offset, &register_address)) {
      return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                              "AIE2P native patch address overflows");
    }
    uint64_t argument_ordinal = 0;
    IREE_RETURN_IF_ERROR(iree_hal_amd_xdna_aie2p_native_query_argument_ordinal(
        image, entry, relocation->binding_ordinal, &argument_ordinal));
    IREE_RETURN_IF_ERROR(
        iree_hal_amd_xdna_aie2p_transaction_writer_append_address_patch(
            writer, register_address, argument_ordinal, relocation->addend));
    ++*relocation_ordinal;
  }
  return iree_ok_status();
}
    ...
    for (uint32_t ordinal = control->first_record_ordinal; ordinal < record_end;
      ...
      IREE_RETURN_IF_ERROR(
          iree_hal_amd_xdna_aie2p_native_append_record_relocations(
              image, entry, control_header, record, &decoded_record,
              relocation_end, &relocation_ordinal, &writer));
    }
  }
  ...
  *out_native_image = NULL;
  ...
  for (iree_host_size_t i = 0; i < entry_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(iree_hal_amd_xdna_aie2p_native_measure_control(
        image, entry, control, &measurement));
    if (!iree_host_size_checked_add(native_byte_length, measurement.byte_length,
                                    &native_byte_length)) {
      return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                              "AIE2P native image storage overflows");
    ...
    const iree_hal_amd_xdna_image_control_program_t* control =
        entry->control_program_header_ordinal == UINT32_MAX
            ...
            : iree_hal_amd_xdna_aie2p_native_find_control(
                  image, entry->control_program_header_ordinal);
    iree_hal_amd_xdna_aie2p_native_measurement_t measurement;
    status = iree_hal_amd_xdna_aie2p_native_measure_control(
        image, entry, control, &measurement);
    if (!iree_status_is_ok(status)) break;
    iree_byte_span_t storage = iree_make_byte_span(
        (uint8_t*)native_image + native_bytes_cursor, measurement.byte_length);
    status = iree_hal_amd_xdna_aie2p_native_encode_control(
        image, target, entry, control, scratch, storage);
    if (!iree_status_is_ok(status)) break;
