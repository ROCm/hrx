...
typedef struct loom_expression_entry_t {
  ...
  uint32_t epoch;
  // Caller-owned availability metadata. The table never interprets this value.
  uint64_t data;
...
enum loom_expression_lookup_flag_bits_e {
  // Restricts lookup to the current block.
  LOOM_EXPRESSION_LOOKUP_FLAG_LOCAL = 1u << 0,
  // Stops at joins and loop headers that can observe another state epoch.
  LOOM_EXPRESSION_LOOKUP_FLAG_STATEFUL = 1u << 1,
};
