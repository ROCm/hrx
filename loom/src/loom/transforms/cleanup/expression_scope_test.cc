...
TEST_F(ExpressionScopeTest,
  ...
  for (;;) {
    ...
    if (cursor.op != root && same_as_root) {
      ...
      EXPECT_EQ(loom_expression_scope_find(&cursor, hash,
                                           LOOM_EXPRESSION_LOOKUP_FLAG_LOCAL),
                nullptr);
    } else {
      EXPECT_EQ(existing, nullptr);
    }
    // A block-local policy can require its own producer even when a semantic
    // equivalent dominates. It must not change bindings in sibling blocks.
    loom_expression_scope_insert(&cursor, {cursor.op, hash, cursor.epoch, 0});
  }
  EXPECT_EQ(visited, 7);
...
TEST_F(ExpressionScopeTest,
  ...
  loom_expression_scope_insert(&cursor, {outer, hash, cursor.epoch, 100});
  Next(isolated);
  cursor = Next(inner);
  EXPECT_EQ(loom_expression_scope_find(&cursor, hash, 0), nullptr);
  loom_expression_scope_insert(&cursor, {inner, hash, cursor.epoch, 200});
  Next(branch);
  cursor = Next(dominated);
  auto* existing = loom_expression_scope_find(&cursor, hash, 0);
  ASSERT_NE(existing, nullptr);
  EXPECT_EQ(existing->op, inner);
  EXPECT_EQ(existing->data, 200u);
  cursor = Next(resumed);
  existing = loom_expression_scope_find(&cursor, hash, 0);
  ASSERT_NE(existing, nullptr);
  EXPECT_EQ(existing->op, outer);
  EXPECT_EQ(existing->data, 100u);
  Next(nullptr);
}
...
TEST_F(ExpressionScopeTest, ReplacingAnExpiredLocalBindingKeepsRollbackIntact) {
  ...
  auto cursor = Next(outer);
  loom_expression_scope_insert(&cursor, {outer, hash, cursor.epoch, 0});
  Next(map);
  const iree_host_size_t initial_bytes = arena_.used_allocation_size;
  cursor = Next(first);
  loom_expression_scope_insert(&cursor, {first, hash, cursor.epoch, 10});
  cursor = Next(collision);
  loom_expression_scope_insert(&cursor, {collision, hash, cursor.epoch, 20});
  cursor = Next(second);
  // Epoch policy can expire a producer without removing its structural key.
  loom_expression_scope_insert(&cursor, {second, hash, cursor.epoch, 30});
  cursor = Next(third);
  auto* existing = loom_expression_scope_find(&cursor, hash, 0);
  ASSERT_NE(existing, nullptr);
  EXPECT_EQ(existing->op, second);
  EXPECT_EQ(existing->data, 30u);
...
TEST_F(ExpressionScopeTest, InventoryIncludesOperationsAfterEmptyNestedBlocks) {
  ...
  for (auto* op : {first, second, last}) {
    ...
    loom_expression_scope_insert(
        &cursor, {op, loom_expression_hash(module_, op), cursor.epoch, 0});
  }
  Next(nullptr);
}
