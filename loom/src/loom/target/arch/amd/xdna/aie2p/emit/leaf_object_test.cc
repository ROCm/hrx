    ...
    for (iree_string_view_t name : register_names) {
      ...
      const loom_low_allocation_assignment_t* assignment =
          loom_low_packet_descriptor_operand_assignment(
              &leaf.frame.allocation, &packet, operand_index++);
      ASSERT_NE(assignment, nullptr);
      EXPECT_EQ(assignment->location_kind,
                LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER);
      EXPECT_EQ(assignment->location_base,
...
TEST_F(Aie2pLeafObjectTest, LowFunctionsEmitOptimizedVectorLeaves) {
  // RET shares the add's bundle. Its five delay slots cover the final store's
  // outgoing memory-event separation before arbitrary successor instructions,
  // including lock operations, may execute.
  // The two loaded vectors pack into x0/x1, preserving other wide views.
  // LLVM-AIE independently assembles each complete physical sequence below.
  struct TestCase {
    std::string_view vector_shape;
    std::string_view mnemonic;
    std::array<uint8_t, 36> expected;
  };
  const TestCase test_cases[] = {
      {
          "i32x16",
          "vadd.32",
          {0x3c, 0xe8, 0x08, 0x72, 0x83, 0x00, 0x00, 0x00, 0x00,
           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
           0xe4, 0x5a, 0x10, 0x00, 0x00, 0x05, 0x00, 0x00, 0x18,
           0x13, 0x04, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
      },
      {
          "i16x32",
          "vadd.16",
          {0x3c, 0xe8, 0x08, 0x72, 0x83, 0x00, 0x00, 0x00, 0x00,
           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
           0xe4, 0x3a, 0x10, 0x00, 0x00, 0x05, 0x00, 0x00, 0x18,
           0x13, 0x04, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
      },
      {
          "i8x64",
          ...
          {0x3c, 0xe8, 0x08, 0x72, 0x83, 0x00, 0x00, 0x00, 0x00,
           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
           0xe4, 0x1a, 0x10, 0x00, 0x00, 0x05, 0x00, 0x00, 0x18,
           0x13, 0x04, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
      },
  };
  ...
  for (const TestCase& test_case : test_cases) {
    ...
    ASSERT_EQ(leaf.plan.bundle_count, 13u);
    ASSERT_EQ(leaf.plan.slot_count, 15u);
    ASSERT_EQ(leaf.plan.encoded_byte_length, 36u);
    ...
    for (iree_host_size_t i = 0; i < leaf.plan.slot_count; ++i) {
      ...
      if (iree_any_bit_set(slot->flags,
        ...
        EXPECT_EQ(leaf.plan.bundles[7].slot_start + 1, i);
        EXPECT_NE(slot->scheduled_packet_index,
                  LOOM_AIE2P_BUNDLE_PLAN_PACKET_NONE);
      }
    }
    EXPECT_EQ(synthetic_nop_count, 10u);
    ...
    EXPECT_TRUE(iree_string_view_equal(leaf.contribution.object.symbols[0].name,
                                       IREE_SV("vector_add")));
    EXPECT_EQ(leaf.contribution.object.symbols[0].size, 36u);
...
TEST_F(Aie2pLeafObjectTest, FifoLoadStateAllocatesAsOnePhysicalTuple) {
  // Pinned Peano independently establishes the physical instruction
  // encodings for the same unaligned 512-bit load. Loom chains a second pop
  // through the updated tuple and selects the compact four-byte standalone
  // bundle instead of Peano's move-plus-two-NOP bundle.
  constexpr std::array<uint8_t, 32> kExpected = {
      0x18, 0xc0, 0x00, 0x00, 0x18, 0x14, 0x14, 0x3c, 0x18, 0x14, 0x00,
      0x38, 0x18, 0x14, 0x00, 0x38, 0x00, 0x00, 0x18, 0x00, 0x28, 0x10,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  };
  CompiledLeaf leaf;
  IREE_ASSERT_OK(CompileSource(
      "low.func.def target<amd.xdna.aie2p.core> @fifo_load("
      "%pointer: reg<aie2p.eps>, %initial_fifo: reg<aie2p.eldfiforeg>) "
      "-> (reg<aie2p.vec256 x2>) asm {\n"
      "  %position = mova.fifo.position 0\n"
      "  %filled_pointer, %fifo, %filled_position = vldb.fill.512 "
      "%pointer, %initial_fifo, %position\n"
      "  %first_data, %next_pointer, %next_fifo, %next_position = "
      "vldb.pop.512.i8x64 %filled_pointer, %fifo, %filled_position\n"
      "  %data, %final_pointer, %final_fifo, %final_position = "
      "vldb.pop.512.i8x64 %next_pointer, %next_fifo, %next_position\n"
      "  return %data\n"
      "}\n",
      &leaf));
  ...
  const loom_low_packet_view_t* fill = nullptr;
  std::array<loom_low_packet_view_t, 2> pop_storage = {};
  iree_host_size_t pop_count = 0;
  loom_low_packet_view_t fill_storage = {};
  for (iree_host_size_t packet_index = 0;
    ...
    if (packet.descriptor ==
        &descriptor_set
             ->descriptors[AIE2P_CORE_DESCRIPTOR_REF_LOAD_B_FIFO_FILL_512]) {
      fill_storage = packet;
      fill = &fill_storage;
    } else if (packet.descriptor ==
               &descriptor_set->descriptors
                    [AIE2P_CORE_DESCRIPTOR_REF_LOAD_B_I8X64_FIFO_POP]) {
      ASSERT_LT(pop_count, pop_storage.size());
      pop_storage[pop_count++] = packet;
    }
  }
  ASSERT_NE(fill, nullptr);
  ASSERT_EQ(pop_count, pop_storage.size());

  ExpectPhysicalRegisters(leaf, *fill,
                          {IREE_SV("p0"), IREE_SV("lf0"), IREE_SV("r24"),
                           IREE_SV("p0"), IREE_SV("lf0"), IREE_SV("r24")});
  for (const loom_low_packet_view_t& pop : pop_storage) {
    ExpectPhysicalRegisters(
        leaf, pop,
        {IREE_SV("x0"), IREE_SV("p0"), IREE_SV("lf0"), IREE_SV("r24"),
         IREE_SV("p0"), IREE_SV("lf0"), IREE_SV("r24")});
  }
...
TEST_F(Aie2pLeafObjectTest, IndependentFifoRefillsPreserveBothPhysicalTuples) {
  // Both incoming FIFO states are live. Reversing their parameter order
  // relative to the pointers prevents independent first-free allocations
  // from accidentally satisfying the shared hardware selectors.
  CompiledLeaf leaf;
  IREE_ASSERT_OK(CompileSource(
      "low.func.def target<amd.xdna.aie2p.core> @fifo_pair("
      "%pointer0: reg<aie2p.eps>, %pointer1: reg<aie2p.eps>, "
      "%fifo1: reg<aie2p.eldfiforeg>, %fifo0: reg<aie2p.eldfiforeg>, "
      "%position1: reg<aie2p.erf2>, %position0: reg<aie2p.erf2>) "
      "-> (reg<aie2p.vec256 x2>) asm {\n"
      "  %p0, %f0, %a0 = vlda.fill.512 %pointer0, %fifo0, %position0\n"
      "  %p1, %f1, %a1 = vldb.fill.512 %pointer1, %fifo1, %position1\n"
      "  %lhs, %next_p0, %next_f0, %next_a0 = "
      "vlda.pop.512.i32x16 %p0, %f0, %a0\n"
      "  %rhs, %next_p1, %next_f1, %next_a1 = "
      "vldb.pop.512.i32x16 %p1, %f1, %a1\n"
      "  %sum = vadd.32 %lhs, %rhs\n"
      "  return %sum\n"
      "}\n",
      &leaf));
  ...
  for (iree_host_size_t packet_index = 0;
    ...
    if (packet.descriptor ==
        &descriptor_set
             ->descriptors[AIE2P_CORE_DESCRIPTOR_REF_LOAD_A_FIFO_FILL_512]) {
      ...
      ++fill_count;
               ...
               &descriptor_set->descriptors
                    [AIE2P_CORE_DESCRIPTOR_REF_LOAD_B_FIFO_FILL_512]) {
      ExpectPhysicalRegisters(leaf, packet,
                              {IREE_SV("p1"), IREE_SV("lf1"), IREE_SV("r25"),
                               IREE_SV("p1"), IREE_SV("lf1"), IREE_SV("r25")});
      ++fill_count;
    }
  }
...
TEST_F(Aie2pLeafObjectTest, ResourceImportsAnchorRegistersWithoutEmittingCode) {
  constexpr std::array<uint8_t, 36> kExpected = {
      0x3c, 0xe8, 0x08, 0x72, 0x83, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe4, 0x5a, 0x01, 0x00, 0x00, 0x05,
      0x00, 0x00, 0x18, 0x13, 0x04, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  };
  CompiledLeaf leaf;
...
TEST_F(Aie2pLeafObjectTest, MaterializesVec256ConcatsAsNativeMoves) {
  ...
  IREE_ASSERT_OK(CompileSource(
      ...
      "  %low = vshuffle %broadcast, %broadcast, %low_config\n"
      "  %high_config = mova.i32 53\n"
      "  %high = vshuffle %broadcast, %broadcast, %high_config\n"
...
TEST_F(Aie2pLeafObjectTest, AllocatedConcatMovesWaitForLoadedData) {
  CompiledLeaf leaf;
  IREE_ASSERT_OK(CompileSource(
      "low.func.def target<amd.xdna.aie2p.core> @duplicate_load(\n"
      "    %source: reg<aie2p.ep>, %destination: reg<aie2p.ep>) asm {\n"
      "  %value = vlda.256.i32x8 %source, 0\n"
      "  %pair = concat(%value, %value) : (reg<aie2p.vec256>, "
      "reg<aie2p.vec256>) -> reg<aie2p.vec256 x2>\n"
      "  vst.512.i32x16 %pair, %destination, 0\n"
      "  return\n"
      "}\n",
      &leaf, LOOM_LOW_SCHEDULE_STRATEGY_SOURCE_PRIORITY));

  uint32_t load_cycle = UINT32_MAX;
  uint32_t move_count = 0;
  for (iree_host_size_t i = 0; i < leaf.plan.bundle_count; ++i) {
    const auto& bundle = leaf.plan.bundles[i];
    for (uint8_t j = 0; j < bundle.slot_count; ++j) {
      const auto& slot = leaf.plan.slots[bundle.slot_start + j];
      if (slot.encoded_slot.slot == LOOM_AIE2P_SLOT_LDA) {
        load_cycle = bundle.issue_cycle;
      }
      if (iree_any_bit_set(slot.flags,
                           LOOM_AIE2P_PLANNED_SLOT_FLAG_STRUCTURAL_MOVE)) {
        ++move_count;
        ASSERT_NE(load_cycle, UINT32_MAX);
        // II_VLDA_dmw_lda_w_idx_imm writes at cycle 8; II_VMOV_alu_mv_mv_w
        // reads at cycle 2, with no forwarding: 8 - 2 + 1 = 7 cycles.
        EXPECT_GE(bundle.issue_cycle - load_cycle, 7u);
      }
    }
  }
  EXPECT_GT(move_count, 0u);
  EXPECT_GT(leaf.plan.issue_cycle_count, leaf.plan.bundle_count);
  const auto bytes = leaf.contribution.object.sections[0].contents;
  uint32_t prior_end_offset = 0;
  uint32_t prior_end_cycle = 0;
  for (iree_host_size_t i = 0; i < leaf.plan.bundle_count; ++i) {
    const auto& bundle = leaf.plan.bundles[i];
    EXPECT_EQ(bundle.byte_offset - prior_end_offset,
              2 * (bundle.issue_cycle - prior_end_cycle));
    for (uint32_t offset = prior_end_offset; offset < bundle.byte_offset;
         ++offset)
      EXPECT_EQ(bytes.data[offset], 0);
    loom_aie2p_bundle_format_info_t format;
    ASSERT_TRUE(
        loom_aie2p_encoding_query_bundle_format_info(bundle.format, &format));
    prior_end_offset = bundle.byte_offset + format.bit_count / 8;
    prior_end_cycle = bundle.issue_cycle + 1;
  }
  EXPECT_EQ(prior_end_cycle, leaf.plan.issue_cycle_count);
  ResetLeaf(&leaf);
}
