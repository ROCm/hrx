  ...
  void CheckCursor(const loom_low_allocation_move_plan_t& plan,
    ...
    loom_region_for_each_block(region, block) {
      ...
      loom_block_for_each_op(block, op) {
        ...
        ++*visited_count;
        if (!loom_liveness_analysis_includes_region_tree(&liveness)) continue;
        loom_region_t* const* regions = loom_op_regions(op);
        for (uint8_t i = 0; i < op->region_count; ++i) {
          if (regions[i]) CheckCursor(plan, regions[i], cursor, visited_count);
        }
      }
    }
  }
...
^exit(%returned: reg<test.i32>):
  ...
  for (const auto* schedule : schedules) {
    const loom_low_allocation_move_plan_context_t context = {
        .assignment_map = {.liveness = &frame.allocation.liveness},
        .schedule = schedule,
    };
