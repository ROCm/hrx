...
TEST_F(LowEmissionFrameTest, ReusedRegisterWaitsForPreviousPhysicalRead) {
  for (const auto strategy : kScheduleStrategies) {
...
low.func.def target<test.low.core> @physical_reuse(%seed: reg<test.phys>) -> (reg<test.phys>) asm {
  ...
  return %next
    ...
    const loom_low_allocation_fixed_value_t fixed_values[] = {
        {body->arg_ids[0], LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER, 1,
         1},
    ...
    loom_low_emission_frame_t frame = {};
    IREE_ASSERT_OK(loom_low_emission_frame_build(module.get(), function,
                                                 &options, &arena_, &frame));
    ASSERT_EQ(frame.schedule.error_count, 0u);
    ASSERT_EQ(frame.allocation.error_count, 0u);
    loom_low_physical_issue_t issue = {};
    IREE_ASSERT_OK(loom_low_physical_issue_initialize(
        frame.target.descriptor_set, &arena_, &issue));
    uint32_t cycles[3] = {};
    ...
    for (uint32_t i = 0; i < frame.schedule.scheduled_node_count; ++i) {
      ...
      IREE_ASSERT_OK(loom_low_physical_issue_place(
          &issue, &instruction, 1,
          iree_max(next_cycle, packet.node->issue_cycle), &cycle));
    ...
    EXPECT_FALSE(loom_low_physical_issue_group_fits(frame.target.descriptor_set,
                                                    pair, 2));
    write_registers[0] = 1;
    EXPECT_TRUE(loom_low_physical_issue_group_fits(frame.target.descriptor_set,
                                                   pair, 2));
  }
}
...
TEST_F(LowEmissionFrameTest, MaterializationFillsReadyLatencyWindow) {
  ModulePtr module = ParseModule(R"(
low.func.def target<test.low.core> @constant_latency(%lhs: reg<test.i32>, %rhs: reg<test.i32>) -> (reg<test.i32>) asm {
  ...
  EXPECT_LT(frame.schedule.nodes[1].scheduled_ordinal,
            frame.schedule.nodes[2].scheduled_ordinal);
  EXPECT_LT(frame.schedule.nodes[1].issue_cycle,
            frame.schedule.nodes[2].issue_cycle);
  EXPECT_EQ(frame.schedule.nodes[3].issue_cycle,
            frame.schedule.nodes[2].issue_cycle + 1);
}
