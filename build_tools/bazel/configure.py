...
class ConfigRequest:
    ...
    def amdf_build_enabled(self) -> bool:
        if self.amdf_build is not None:
            return self.amdf_build
        return "amdgpu" in self.enabled_drivers
...
def generate_config(args: argparse.Namespace) -> str:
    ...
    lines = [
        ...
        bazelrc_line(
            "build",
            "--//libamdf/config:enabled=" + str(request.amdf_build_enabled()).lower(),
        ),
        bazelrc_line(
            "build",
