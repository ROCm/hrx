...
amdf_status_t amdf_gpu_umd_query_endpoint_profile(
  ...
  *out_available = false;
  ...
  if (amdf_status_is_ok(status)) {
    status = amdf_gpu_wddm_wkmi_loader_query_properties(
        &loader, platform_endpoint->adapter,
        platform_endpoint->physical_adapter_index, &provider_properties,
        &provider_properties_available);
  }
  ...
  if (loader.module != NULL) {
    const amdf_status_t deinitialize_status =
        amdf_gpu_wddm_wkmi_loader_deinitialize(&loader);
