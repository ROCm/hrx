...
struct amdf_wkmi_bridge_gpu_kernel_queue_t {
  ...
  // Intrusive link used only after a failed construction rollback.
  amdf_wkmi_bridge_gpu_kernel_queue_t* next_deferred = nullptr;
...
void DeferQueueRollback(amdf_wkmi_bridge_gpu_adapter_t* adapter,
                        amdf_wkmi_bridge_gpu_kernel_queue_t* queue) {
  AcquireSRWLockExclusive(&adapter->queue_lock);
  queue->next_deferred = adapter->deferred_queue_head;
  adapter->deferred_queue_head = queue;
...
amdf_wkmi_bridge_result_t CreateNativeQueue(
  ...
  if (native_status != STATUS_SUCCESS) {
    ...
    if (rollback_status != STATUS_SUCCESS) {
      DeferQueueRollback(adapter, queue.release());
...
amdf_wkmi_bridge_result_t PrepareGpuAdapterClose(
  ...
  *out_native_status = 0;
  ...
  while (adapter->deferred_queue_head != nullptr) {
    amdf_wkmi_bridge_gpu_kernel_queue_t* queue = adapter->deferred_queue_head;
    const NTSTATUS native_status = DestroyNativeQueue(queue);
    if (native_status != STATUS_SUCCESS) {
      ReleaseSRWLockExclusive(&adapter->queue_lock);
      return MakeNativeFailure(native_status, out_native_status);
    }
    adapter->deferred_queue_head = queue->next_deferred;
    delete queue;
  }
  ReleaseSRWLockExclusive(&adapter->queue_lock);
  return AMDF_WKMI_BRIDGE_RESULT_SUCCESS;
