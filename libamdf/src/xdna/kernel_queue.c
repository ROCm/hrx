...
typedef struct amdf_xdna_kernel_queue_t {
  ...
  // Command borrowed by the pending submission.
  amdf_xdna_command_t* pending_command;
...
// including release of its command borrow.
static bool amdf_xdna_kernel_queue_try_retire(amdf_xdna_kernel_queue_t* queue,
  ...
  amdf_xdna_command_unregister_submission(queue->pending_command);
  queue->pending_command = NULL;
...
static amdf_status_t amdf_xdna_kernel_queue_wait(
  ...
  for (;;) {
    ...
    if (amdf_xdna_kernel_queue_try_retire(queue, submission)) {
      return AMDF_STATUS_OK;
    }
    ...
    if (occupancy == AMDF_XDNA_KERNEL_QUEUE_OCCUPANCY_RETIRING) {
      // Another thread has already observed native completion and is releasing
      // the command borrow. This critical section contains only bounded host
      // bookkeeping; wait for its retirement publication before returning.
      continue;
    }
    ...
    const amdf_status_t status = amdf_xdna_umd_kernel_queue_wait(
...
amdf_status_t AMDF_CALL amdf_xdna_kernel_queue_create(
  ...
  *out_queue = NULL;
  ...
  amdf_xdna_kernel_queue_t* queue =
      (amdf_xdna_kernel_queue_t*)calloc(1, sizeof(*queue));
  if (queue == NULL) {
  ...
  if (amdf_status_is_ok(status)) {
    status = amdf_xdna_umd_kernel_queue_create(amdf_xdna_device_get_umd(device),
                                               &queue->umd);
  }
  if (amdf_status_is_ok(status)) {
...
amdf_status_t AMDF_CALL amdf_xdna_kernel_queue_submit(
  ...
  if (submission_info->reserved != 0 || submission_info->command_count != 1 ||
      ...
      submission_info->commands[0] == NULL) {
    return amdf_make_api_status(AMDF_STATUS_CODE_INVALID_ARGUMENT);
  }

  amdf_xdna_kernel_queue_t* queue = (amdf_xdna_kernel_queue_t*)base_queue;
  amdf_xdna_command_t* command = submission_info->commands[0];
  if (amdf_xdna_program_get_device(amdf_xdna_command_get_program(command)) !=
      base_queue->device) {
    return amdf_make_api_status(AMDF_STATUS_CODE_INVALID_ARGUMENT);
  }
  if (amdf_xdna_command_get_umd(command) == NULL ||
  ...
  status = amdf_xdna_command_register_submission(command);
  const bool command_registered = amdf_status_is_ok(status);
  uint64_t native_submission = 0;
  if (amdf_status_is_ok(status)) {
    status = amdf_xdna_umd_kernel_queue_submit(
        queue->umd, amdf_xdna_command_get_umd(command), &native_submission);
  }
  if (!amdf_status_is_ok(status)) {
    if (command_registered) {
      amdf_xdna_command_unregister_submission(command);
    }
    amdf_atomic_uint64_store_release(
        &queue->slot_state,
        amdf_xdna_kernel_queue_make_slot_state(
